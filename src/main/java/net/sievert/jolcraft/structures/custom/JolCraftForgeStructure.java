package net.sievert.jolcraft.structures.custom;
...
public class JolCraftForgeStructure extends Structure {

    // A custom codec that changes the size limit for our code_structure_sky_fan.json's config to not be capped at 7.
    // With this, we can have a structure with a size limit up to 30 if we want to have extremely long branches of pieces in the structure.
    public static final Codec<JolCraftForgeStructure> CODEC = RecordCodecBuilder.<JolCraftForgeStructure>mapCodec(instance ->
            instance.group(JolCraftForgeStructure.settingsCodec(instance),
            ...
            ).apply(instance, JolCraftForgeStructure::new)).codec();
    ...
    public JolCraftForgeStructure(Structure.StructureSettings config,
        ...
        /*for (int i = -30; i < 30; i++) {
            if (
                    ...
                    context.chunkGenerator().getFirstOccupiedHeight(
                            chunkpos.getMinBlockX() + i,
                            chunkpos.getMinBlockZ(),
                            Heightmap.Types.WORLD_SURFACE_WG,
                            context.heightAccessor(),
                            context.randomState()) > 62

                            &&

                            context.chunkGenerator().getFirstOccupiedHeight(
                                    chunkpos.getMinBlockX(),
                                    chunkpos.getMinBlockZ() + i,
                                    Heightmap.Types.WORLD_SURFACE_WG,
                                    context.heightAccessor(),
                                    context.randomState()) > 62
    ...
    public Optional<Structure.GenerationStub> findGenerationPoint(Structure.GenerationContext context) {
        ...
        if (!JolCraftForgeStructure.extraSpawningChecks(context)) {
    ...
    public StructureType<?> type() {
        return JolCraftStructures.FORGE.get(); // Helps the game know how to turn this structure back to json to save to chunks
    }
}