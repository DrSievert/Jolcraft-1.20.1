...
public class DwarfWarrior extends DwarfEntity {
    public DwarfWarrior(EntityType<? extends DwarfEntity> entityType, Level level) {
        super(entityType, level);
    }

    public static AttributeSupplier setAttributes() {
        return createMobAttributes().add(Attributes.MAX_HEALTH, 40.0D).add(Attributes.KNOCKBACK_RESISTANCE, 0.8).add(Attributes.MOVEMENT_SPEED, (double)0.35F).add(Attributes.ATTACK_DAMAGE, 2.0D).add(Attributes.ARMOR, 20.0D).add(Attributes.ARMOR_TOUGHNESS, 9.0D).build();
    }
    @Nullable
    @Override
    public SpawnGroupData finalizeSpawn(ServerLevelAccessor levelAccessor, DifficultyInstance difficultyInstance, MobSpawnType spawnType, @Nullable SpawnGroupData spawnGroupData, @Nullable CompoundTag compoundTag) {
        ...
        this.setVillagerData(this.getVillagerData().setProfession(JolCraftVillagers.WARRIOR.get()));
